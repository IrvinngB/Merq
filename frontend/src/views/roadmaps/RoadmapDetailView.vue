...
async function autoLayoutNodes() {
  ...
  try {
    await aiApi.autoLayout(roadmapsStore.currentRoadmap.id)
    await roadmapsStore.fetchRoadmap(roadmapId.value)
  } catch (err) {
    console.error('Error auto-layout:', err)
  } finally {
            ...
            <div class="flex flex-wrap items-center gap-4">
              ...
              <div class="flex items-center gap-2">
                ...
                <button
                  ...
                  class="p-2 rounded-lg bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 transition-colors disabled:opacity-50"
                  title="Reorganizar nodos automáticamente"
                >
                  <svg v-if="autoLayouting" class="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24">
